...
//        }
            ...
            for (int j = 0; j < this.listElements.size(); j++) {
                if (collision(this.listElements.get(i), this.listElements.get(j)) && i != j) {
                    if (this.listElements.get(i).name.equals(this.listElements.get(j).name)) {
                        ...
                        this.listElements.get(i).attack(this.listElements.get(j));
                    }
    ...
    public boolean countSpecies() {
        ...
        int nbWolf = 0;
        for (int i = 0; i < this.listElements.size(); i++) {
            switch (this.listElements.get(i).name) {
                case "Cat":
                    nbCat = 1;
                    break;
                case "Dog":
        ...
        if ((nbCat+nbDog+nbChicken+nbCow+nbWolf)==1){
            return true;
        }else{
            return false;
        }
